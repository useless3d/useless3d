...
void ExampleScene1::load()
{
    ...
    this->getStage(stageId).getCamera()->setPosition(0.0f, 4.0f, 8.0f);
    this->getStage(stageId).getCamera()->setLookAt(0.0f, 0.0f, 0.0f);
    this->getStage(stageId).loadActors("data/models/bin/stages/004/004.fbx"); // add actors to this stage, use default shader for all meshes in this file
